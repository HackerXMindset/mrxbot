...
if __name__ == "__main__":
    ...
    # Start the bot and Flask app concurrently
    if os.getenv("FLASK_ENV") == "development":
        logger.info(f"Running Flask app in development mode on port {PORT}")
        import threading
        flask_thread = threading.Thread(target=lambda: app.run(host="0.0.0.0", port=PORT, debug=True, use_reloader=False))
        flask_thread.start()
        asyncio.run(start_bot())
    else:
        logger.info("Running in production mode (expecting gunicorn to start the app)")
        # Start the bot tasks in a separate thread to allow gunicorn to run
        import threading
