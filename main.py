...
from utils import calculate_hitrate, format_value, format_percentage, format_time_diff
...
async def handle_test(message: types.Message):
    ...
    if len(args) < 3: return await message.reply("Usage: /test {contract_address} {market_cap|bonded|hypothetical} [value]")
    address, test_type = args[1], args[2].lower()
    from bot import fetch_market_cap, fetch_bonding_status

    if test_type == "market_cap":
        mc_str, mc_value, *_ = await fetch_market_cap(address, datetime.now(timezone.utc))
